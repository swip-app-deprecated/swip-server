...
fn main() {
    ...
    server::new(move || {
        App::with_state(AppState {
            ...
            .resource("/graphql", |r| {
                r.method(http::Method::POST).h(graphql::executor::graphql)
            })
            .resource("/graphiql", |r| {
                r.method(http::Method::GET).h(graphql::executor::graphiql)
            })
    ...
    println!(
        "\n {} {}\n",
        " ".blink().green(),
        "Started http server: localhost:8000".bold().yellow()
    );
    let _ = sys.run();
    println!(
        "\n {} {}\n",
        "".blink().red(),
        "Server stopped".bold().yellow()
    )
}
